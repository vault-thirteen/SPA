...
function composeURL(protocol, hostPort, path) {
...
async function getJsonObject(pagePath) {
	...
	let url = composeURL(mca.server.json.protocol, mca.server.json.hostPort, pagePath);
...
function fillPageWithContent(obj) {
	...
	tag.innerHTML = tagH1(obj.Title) + "\r\n" +
		tagBlock(tagIcon(obj.Icon, obj.Title) + tagDescription(obj.Description)) + "\r\n" +
		mainContent;
